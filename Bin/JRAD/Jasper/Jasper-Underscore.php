...
function _FORMS ($labelArray, $fieldArray, $placeholder = 'NO', $required = 'YES')
{
	...
	foreach ($fieldArray as $i => $name)
	{
		...
		if (JSP_SORT_GATE($name,'about,article,writeup,content,essay,message,comment,summary,feedback,listing,skills','OR'))
		{
			//TEXTAREA
			$input = JSP_FORMS_TEXTAREA($name,$required);
		}
		else if (JSP_SORT_GATE($name,'gender,sex,year,month,day,soo,origin,state,location,bank,acct_type,trans_type,answer,status,ppa_state','OR'))
		{		
			//SELECT
			...
			else if (JSP_SORT_GATE($name,'soo,origin,state,location,ppa_state','OR'))
		...
		else if (JSP_SORT_GATE($name,'file,upload,profile,image,img','OR'))
		{
			//FILE

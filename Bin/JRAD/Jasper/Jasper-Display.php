...
function JSP_DISPLAY_ERROR ($errorMessage, $errorColor)
{
	...
	if (IS_ERROR)
		$errorMessage = IS_ERROR; //$_GET['JSP_DISPLAY_ERROR']
	if (!$errorColor || !in_array($errorColor,$parseArray))
		$errorColor = $parseArray[2]; //INFO
	if ($errorMessage)
	{
		$output = "<div class='JSP_DISPLAY_ERROR box_shadow $errorColor' id='JSP_DISPLAY_ERROR' style='display:block;'>
			<div class='head'>
...
function JSP_DISPLAY_ILIST ($dtArray, $ddArray)
{
	...
	else
	{
		...
		{
			$dt = "<dt onclick=BLN_DISPLAY_ILIST(".$key.")><span class='sign'>+</span> ".$value."</dt>";
			$dd = "<dd>".$ddArray[$key]."</dd>";
			$body .= $dt.$dd;
		}
		$output = "<dl class='JSP_DISPLAY_ILIST'>".$body."</dl>";
		return $output;
	}
}
...
function JSP_DISPLAY_IOPTION ($array, $selected, $vtype, $function)
{
	$paramArray = array($array,JSP_TRUEPUT($selected),$vtype,JSP_TRUEPUT($function));	
	$parseArray = array('KEY','VALUE');		
	if (JSP_PARAM_FORMAT($paramArray)) 
		return JSPIF;
	else if (JSP_PARAM_PARSE($parseArray,$vtype))
	...
	else
	{
		...
		if (!$function)
			$function = 'RFID'; 
		if ($selected === 0 || !isset($_REQUEST[$function]))
			$selected = 'NULL';			
		if ($selected == 'NULL')
		{
			$output = "<option selected='selected'></option>";			
			foreach ($array as $key => $value)
			{
				$_value = JSP_CONSTRUCT($value,'SCRIPT');
				if ($vtype == $parseArray[0]) //KEY
					$output .= "<option onClick=BLN_ACTION_IOPTION('".$function."','".$key."')>".$value."</option>";
				else
					$output .= "<option onClick=BLN_ACTION_IOPTION('".$function."','".$_value."')>".$value."</option>";
			}
		}
		else
		{
			foreach ($array as $key => $value)	
			{
				...
				{
					if ($key == $selected)	
						$output .= "<option onClick=BLN_ACTION_IOPTION('".$function."','".$key."') selected='selected'>".$value."</option>";
					else
						$output .= "<option onClick=BLN_ACTION_IOPTION('".$function."','".$key."')>".$value."</option>";		
				}
				else
				{
					if ($value == $selected)	
						$output .= "<option onClick=BLN_ACTION_IOPTION('".$function."','".$_value."') selected='selected'>".$value."</option>";
					else
						$output .= "<option onClick=BLN_ACTION_IOPTION('".$function."','".$_value."')>".$value."</option>";				
				}				
			}

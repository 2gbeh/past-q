...
function JSP_SSQL_CLONE ($table, $fieldArray, $entryArray)
{
	...
	else
	{
		...
		return array_search($entryConcat,$recordConcat);
	}
}
...
function JSP_SSQL_IPGET ()
{
	...
	if (JSP_VAL_FILTER('ip',$ip) == 'true')
		return $ip;
	else
		return JSPON;
}

function JSP_SSQL_IPSET ($resource)
{
	$table = JSP_TABLE_VISITOR;
	$entryArray = array(JSP_SSQL_IPGET(),1,JSP_DATE_LONG,JSP_TIME_LONG);					
	if (!JSP_SSQL_EXIST($resource,$table,'TABLE'))
	{
		//CREATE TABLE AND INSERT RECORD
		$fieldArray = array('ip VARCHAR (50)','counter INT (3)','date VARCHAR (16)','time VARCHAR (8)');
		$JSP_SSQL_SCHEMA = JSP_FOOBAR_ITABLE($table,$fieldArray);
		JSP_CRUD_CREATE($table,$entryArray);		
	}
	else
	{
		$JSP_SSQL_CLONE = JSP_SSQL_CLONE($table,'ip,date',array(JSP_SSQL_IPGET(),JSP_DATE_LONG));
		if ($JSP_SSQL_CLONE == 1)
		{
			//IPSET TODAY
			$JSP_FETCH_AND = JSP_FETCH_AND($table,'ip,date',array(JSP_SSQL_IPGET(),JSP_DATE_LONG),1);
			$ASSOC_ID = $JSP_FETCH_AND['id'];
			$COUNTER = $JSP_FETCH_AND['counter'] + 1;
			$entryArray = array($COUNTER,JSP_DATE_LONG,JSP_TIME_LONG);
			JSP_CRUD_UPDATE($table,'counter,date,time',$entryArray,array('PRIKEY',$ASSOC_ID,1));
		}
		else
			JSP_CRUD_CREATE($table,$entryArray);				
	}
...
function JSP_SSQL_IPPARSE ($table, $field)
{
	...
	{
		$JSP_FETCH_RID = JSP_FETCH_RID($table,$field,JSP_SSQL_IPGET());
		if (!JSP_ERROR_CATCH($JSP_FETCH_RID)) 
		{
			if (JSP_ATYPE($JSP_FETCH_RID) == 2)
			{
				foreach ($JSP_FETCH_RID as $assoc_array)
				{
			...
			{
				foreach ($JSP_FETCH_RID as $value)
					$lastValue = $value;	
			}
			return $lastValue;
		}

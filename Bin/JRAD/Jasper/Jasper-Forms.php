...
function JSP_FORMS_TEXTBOX ($value, $placeholder = 'NO', $required = 'YES')
{
	...
	{	
		...
		{
			$type = $value;
			if ($placeholder == $parseArray[1]) //N0
		...
		{
			if ($value == 'dob' && $placeholder == $parseArray[1]) //N0
...
function JSP_FORMS_SELECT ($name, $array, $value = 'KEY', $required = 'YES')
{
	...
	{	
		...
		$output = '<select name="'.$name.'" class="JSP_FORMS_SELECT" id="'.$name.'" '.$required.'>'.
			JSP_DISPLAY_OPTION($array,$_POST[$name],$value)
		.'</select>';		
		return $output;
	}
}
...
function JSP_FORMS_TEXTAREA ($name, $required = 'YES')
{
	...
	{	
		...
		$output = '<textarea class="JSP_FORMS_TEXTAREA" id="'.$name.'" name="'.$name.'" '.$required.'>'.$placeholder.'</textarea>';
		return $output;
	}
}
